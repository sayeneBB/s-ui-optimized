...
install_s-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/sayeneBB/s-ui-optimized/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${yellow}警告：无法获取 s-ui 最新版本（可能是 GitHub API 限制或无发布版本），将使用默认版本 v1.0.0${plain}"
            last_version="v1.0.0"
        fi
        echo -e "已获取 s-ui 最新版本：${last_version}，开始安装..."
        wget -N --no-check-certificate -O /tmp/s-ui-linux-$(arch).tar.gz https://github.com/sayeneBB/s-ui-optimized/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/sayeneBB/s-ui-optimized/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"
